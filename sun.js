...
// Output format THEO YÊU CẦU MỚI
let apiResponseData = {
    "Phien_truoc": "Đang lấy dữ liệu...",
    "Ket_qua_phien_truoc": "...",
    "Xuc_sac_phien_truoc": "...",
    "Phien_nay": "...",
    "Du_doan": "...",
...
class TaiXiuAnalyzer {
    ...
    runSubModel11(results, model) {
        ...
        switch (model.logic) {
            case 'pure':
                if (this.isPerfectAlternating(results, 4)) return { prediction: last === 'Tài' ? 'Xỉu' : 'Tài', confidence: 0.9, reason: 'Phát hiện cầu 1-1 thuần túy' };
                break;
            case 'variant':
                if (this.isAlternatingWithTolerance(results, 1)) return { prediction: last === 'Tài' ? 'Xỉu' : 'Tài', confidence: 0.8, reason: 'Phát hiện cầu 1-1 biến thể' };
                break;
            case 'long':
                const altCount = this.countAlternating(results.slice(-12));
                if (altCount >= 8) return { prediction: last === 'Tài' ? 'Xỉu' : 'Tài', confidence: 0.7 + (altCount / 20), reason: `Cầu 1-1 dài hạn` };
                break;
            case 'hybrid':
                const recent = results.slice(-5);
                if (recent[0] !== recent[1] && recent[1] !== recent[2] && recent[3] !== recent[4]) return { prediction: last === 'Tài' ? 'Xỉu' : 'Tài', confidence: 0.7, reason: 'Phát hiện cầu 1-1 kết hợp' };
                break;
            case 'break':
                if (last4[0] !== last4[1] && last4[1] !== last4[2] && last4[2] !== last4[3]) {
                    if (this.getStreak(results.slice(0, -1)) > 4) return { prediction: last, confidence: 0.8, reason: 'Cầu 1-1 dài sắp gãy, dự đoán giữ nguyên' };
                }
                break;
            case 'recovery':
                if (last4[0] === last4[1] && last4[1] !== last4[2] && last4[2] !== last4[3]) return { prediction: last4[3] === 'Tài' ? 'Xỉu' : 'Tài', confidence: 0.7, reason: 'Cầu 1-1 đang phục hồi sau gãy' };
                break;
        }
        return null;
    }
    ...
    runSubModel22(results, model) {
        ...
        switch (model.logic) {
            case 'pure':
                if (last6.length === 6 && last6[0] === last6[1] && last6[1] !== last6[2] && last6[2] === last6[3] && last6[3] !== last6[4] && last6[4] === last6[5]) return { prediction: last6[4] === 'Tài' ? 'Xỉu' : 'Tài', confidence: 0.9, reason: 'Phát hiện cầu 2-2 chuẩn' };
                break;
            case 'offset':
                if (last6.length === 6 && last6[0] === last6[1] && last6[1] !== last6[2] && last6[2] !== last6[3] && last6[3] === last6[4] && last6[4] !== last6[5]) return { prediction: last6[4] === 'Tài' ? 'Xỉu' : 'Tài', confidence: 0.8, reason: 'Phát hiện cầu 2-2 lệch' };
                break;
            case 'variant':
                if (last8.length === 8 && last8[0] === last8[1] && last8[1] !== last8[2] && last8[2] === last8[3] && last8[3] !== last8[4] && last8[4] === last8[5] && last8[5] !== last8[6] && last8[6] === last8[7]) return { prediction: last8[6] === 'Tài' ? 'Xỉu' : 'Tài', confidence: 0.75, reason: 'Phát hiện cầu 2-2 biến tướng' };
                break;
            case 'hybrid':
                if (last6.length === 6 && last6[0] === last6[1] && last6[1] !== last6[2] && last6[2] !== last6[3] && last6[3] !== last6[4] && last6[4] === last6[5]) return { prediction: last6[4] === 'Tài' ? 'Xỉu' : 'Tài', confidence: 0.7, reason: 'Cầu 2-2 kết hợp 1-1' };
                break;
            case 'long':
                if (last8.length === 8) {
                    let score = 0;
                    for (let i = 0; i < 7; i+=2) if (last8[i] === last8[i+1]) score++;
                    if (score >= 3) return { prediction: last === 'Tài' ? 'Xỉu' : 'Tài', confidence: 0.7 + (score * 0.05), reason: `Cầu 2-2 dài` };
                }
                break;
            case 'break':
                if (last6.length === 6 && last6[0] === last6[1] && last6[1] !== last6[2] && last6[2] === last6[3] && last6[3] !== last6[4] && last6[4] !== last6[5]) return { prediction: last6[4], confidence: 0.85, reason: 'Phát hiện bẻ cầu 2-2' };
                break;
        }
        return null;
    }
    ...
    runSubModelStreak(results, model) {
        ...
        switch (model.logic) {
            case 'short': if (streak >= 2 && streak <= 3) return { prediction: last, confidence: 0.7 + (streak * 0.05), reason: `Bệt ngắn` }; break;
            case 'medium': if (streak >= 4 && streak <= 5) return { prediction: last, confidence: 0.75 + ((streak - 4) * 0.05), reason: `Bệt trung` }; break;
            case 'long': if (streak >= 6) return { prediction: last, confidence: 0.8 + (Math.min(streak, 10) * 0.01), reason: `Bệt dài` }; break;
            case 'break': if (streak >= 4) return { prediction: other, confidence: 0.6 + (streak * 0.03), reason: `Bệt dự đoán sắp gãy` }; break;
            case 'hybrid': if (streak >= 3 && results[results.length - streak - 1] !== last) return { prediction: last, confidence: 0.7, reason: `Bệt sau đảo` }; break;
            case 'super': if (streak >= 8) return { prediction: last, confidence: 0.9, reason: `Siêu bệt` }; break;
        }
        return null;
    }
    ...
    runSubModel33(results, model) {
        ...
        switch (model.logic) {
            case 'pure':
                if (last9.length === 9 && last9[0] === last9[1] && last9[1] === last9[2] && last9[3] === last9[4] && last9[4] === last9[5] && last9[6] === last9[7] && last9[7] === last9[8] && last9[0] !== last9[3] && last9[3] !== last9[6]) return { prediction: last9[6] === 'Tài' ? 'Xỉu' : 'Tài', confidence: 0.9, reason: 'Phát hiện cầu 3-3 chuẩn' };
                break;
            case 'variant':
                if (last12.length === 12) {
                    let score = 0;
                    for (let i = 0; i < 12; i+=3) if (i+2 < 12 && last12[i] === last12[i+1] && last12[i+1] === last12[i+2]) score++;
                    if (score >= 3) return { prediction: last === 'Tài' ? 'Xỉu' : 'Tài', confidence: 0.7 + (score * 0.05), reason: `Cầu 3-3 biến thể` };
                }
                break;
            case 'short':
                if (results.length >= 6) {
                    const last6 = results.slice(-6);
                    if (last6[0] === last6[1] && last6[1] === last6[2] && last6[3] === last6[4] && last6[4] === last6[5]) return { prediction: last6[3] === 'Tài' ? 'Xỉu' : 'Tài', confidence: 0.7, reason: 'Cầu 3-3 ngắn' };
                }
                break;
            case 'hybrid':
                if (last9.length === 9 && last9[0] === last9[1] && last9[1] === last9[2] && last9[3] !== last9[4] && last9[5] === last9[6] && last9[6] === last9[7]) return { prediction: last9[6] === 'Tài' ? 'Xỉu' : 'Tài', confidence: 0.75, reason: 'Cầu 3-3 kết hợp' };
                break;
            case 'break':
                if (last9.length === 9 && last9[0] === last9[1] && last9[1] === last9[2] && last9[3] === last9[4] && last9[4] === last9[5] && last9[6] !== last9[7]) return { prediction: last9[6], confidence: 0.8, reason: 'Phát hiện bẻ cầu 3-3' };
                break;
            case 'long':
                if (results.length >= 15) {
                    ...
                    if (pattern.length >= 4 && pattern[0] !== pattern[1] && pattern[1] !== pattern[2]) return { prediction: pattern[pattern.length-1] === 'Tài' ? 'Xỉu' : 'Tài', confidence: 0.8, reason: 'Cầu 3-3 dài hạn' };
                }
                break;
        }
        return null;
    ...
    runSubModel212(results, model) {
        ...
        switch (model.logic) {
            case 'pure':
                if (last5.length === 5 && last5[0] === last5[1] && last5[1] !== last5[2] && last5[2] !== last5[3] && last5[3] === last5[4] && last5[0] === last5[3]) return { prediction: last5[4] === 'Tài' ? 'Xỉu' : 'Tài', confidence: 0.9, reason: 'Phát hiện cầu 2-1-2 chuẩn' };
                break;
            case 'variant':
                if (last7.length === 7 && last7[0] === last7[1] && last7[1] !== last7[2] && last7[3] === last7[4] && last7[4] !== last7[5] && last7[0] === last7[3]) return { prediction: last7[5] === 'Tài' ? 'Xỉu' : 'Tài', confidence: 0.8, reason: 'Phát hiện cầu 2-1-2 biến thể' };
                break;
            case 'long':
                if (results.length >= 10) {
                    ...
                    if (count >= 2) return { prediction: last === 'Tài' ? 'Xỉu' : 'Tài', confidence: 0.75, reason: 'Cầu 2-1-2 dài hạn' };
                }
                break;
        }
        return null;
    ...
    runSubModel121(results, model) {
        ...
        switch (model.logic) {
            case 'pure':
                if (last5.length === 5 && last5[0] !== last5[1] && last5[1] === last5[2] && last5[2] !== last5[3] && last5[3] === last5[4] && last5[0] === last5[3]) return { prediction: last5[4] === 'Tài' ? 'Xỉu' : 'Tài', confidence: 0.9, reason: 'Phát hiện cầu 1-2-1 chuẩn' };
                break;
            case 'variant':
                if (last7.length === 7 && last7[0] !== last7[1] && last7[1] === last7[2] && last7[3] !== last7[4] && last7[4] === last7[5] && last7[0] === last7[3]) return { prediction: last7[5] === 'Tài' ? 'Xỉu' : 'Tài', confidence: 0.8, reason: 'Phát hiện cầu 1-2-1 biến thể' };
                break;
            case 'long':
                if (results.length >= 10) {
                    const last10 = results.slice(-10);
                    let count = 0;
                    for (let i = 0; i < 5; i+=2) if (i+4 < 10 && last10[i] !== last10[i+1] && last10[i+1] === last10[i+2] && last10[i+3] === last10[i+4]) count++;
                    if (count >= 2) return { prediction: last === 'Tài' ? 'Xỉu' : 'Tài', confidence: 0.75, reason: 'Cầu 1-2-1 dài hạn' };
                }
                break;
        }
        return null;
    ...
    runSubModelBreak(results, model) {
        ...
        switch (model.logic) {
            ...
            case 'breakStreak':
                ...
                if (streak >= 3 && last !== results[results.length - 2]) return { prediction: last, confidence: 0.8, reason: `Phát hiện bẻ cầu bệt` };
                break;
            case '11to22':
    ...
    runSubModelAdvanced(results, model) {
        ...
        switch (model.logic) {
            case 'frequency':
                ...
                if (freq.dominant && freq.ratio > 0.6) return { prediction: freq.dominant, confidence: 0.6 + (freq.ratio * 0.2), reason: `Tần suất lệch` };
                break;
            case 'cycle':
                const cycle = this.detectCycle(results);
                if (cycle.found) return { prediction: cycle.next, confidence: 0.7, reason: `Phát hiện chu kỳ` };
                break;
            case 'symmetry':
            ...
            case 'longTrend':
                ...
                if (trend.strength > 0.7) return { prediction: trend.direction, confidence: 0.7 + (trend.strength * 0.1), reason: `Xu hướng dài` };
                break;
            case 'super':
    ...
    runMiniModel(index, history) {
        ...
        switch (miniModel.specialty) {
            ...
            case 'tinh_toan_xac_suat_cao':
                ...
                if (taiCount > xiuCount * 1.5) { prediction = 'Xỉu'; confidence = 0.7; reason = 'Xác suất Tài cao, dự đoán Xỉu để cân bằng'; }
                else if (xiuCount > taiCount * 1.5) { prediction = 'Tài'; confidence = 0.7; reason = 'Xác suất Xỉu cao, dự đoán Tài để cân bằng'; }
            ...
            case 'phan_tich_so_sanh':
                ...
                for (let [type, patterns] of Object.entries(this.patternLibrary)) {
                    if (patterns.includes(currentPattern)) {
                        matchFound = true; prediction = results[results.length - 1] === 'Tài' ? 'Xỉu' : 'Tài';
                        confidence = 0.75; reason = `Khớp mẫu ${type} trong thư viện`; break;
                    }
                }
                if (!matchFound) { prediction = results[results.length - 1]; confidence = 0.4; reason = 'Không tìm thấy mẫu tương tự'; }
                break;
            default:
                const random = Math.random();
                if (random < 0.4) { prediction = results[results.length - 1]; confidence = 0.5; }
                else if (random < 0.7) { prediction = results[results.length - 1] === 'Tài' ? 'Xỉu' : 'Tài'; confidence = 0.5; }
                else {
                    const streak = this.getStreak(results);
                    if (streak >= 3) { prediction = results[results.length - 1]; confidence = 0.6; }
                    else { prediction = results[results.length - 1] === 'Tài' ? 'Xỉu' : 'Tài'; confidence = 0.5; }
                }
                reason = `Mini model ${index}`;
        }
    ...
    analyzeBasicPatterns(history) {
        ...
        return { prediction: bestPattern.prediction, confidence: bestPattern.confidence, pattern_type: bestKey, reason: `Phát hiện cầu ${bestKey}` };
    }
    ...
    analyzeTrend(history) {
        ...
        const results = this.getResultArray(history);
        const shortTrend = this.getMostCommon(this.countResults(results.slice(-3)));
        const longTrend = this.getMostCommon(this.countResults(results.slice(-10)));
        const momentum = this.calculateMomentum(results);
        
        if (shortTrend.count >= 2 && longTrend.count >= 6) return { prediction: shortTrend.value, confidence: Math.min(0.7 + momentum * 0.1, 0.95) };
        else if (shortTrend.count >= 2) return { prediction: shortTrend.value, confidence: Math.min(0.6 + momentum * 0.1, 0.95) };
        else if (longTrend.count >= 6) return { prediction: longTrend.value, confidence: Math.min(0.6 + momentum * 0.1, 0.95) };
        else {
            const other = results[results.length - 1] === 'Tài' ? 'Xỉu' : 'Tài';
            return { prediction: other, confidence: 0.5 };
        }
    }
    ...
    calculateMomentum(results) {
        ...
        const taiCount = results.slice(-5).filter(r => r === 'Tài').length;
    ...
    analyzeImbalance(history) {
        ...
        if (imbalanceRatio > 0.4) {
            return { prediction: countTai > countXiu ? 'Xỉu' : 'Tài', confidence: Math.min(0.7 + imbalanceRatio * 0.2, 0.95) };
        } else {
    ...
    analyzeShortTerm(history) {
        ...
        if (last3[0] !== last3[1] && last3[1] === last3[2]) patterns.push({ type: '1-2', prediction: last3[2] === 'Tài' ? 'Xỉu' : 'Tài', confidence: 0.65 });
        
        if (results.length >= 4) {
            const last4 = results.slice(-4);
            if (last4[0] !== last4[1] && last4[1] !== last4[2] && last4[2] !== last4[3]) patterns.push({ type: 'xen_kẽ', prediction: last4[3] === 'Tài' ? 'Xỉu' : 'Tài', confidence: 0.8 });
        }
        
        if (patterns.length > 0) {
            const bestPattern = patterns.reduce((best, current) => current.confidence > best.confidence ? current : best);
            return { prediction: bestPattern.prediction, confidence: bestPattern.confidence, pattern: bestPattern.type };
        } else {
            return { prediction: results[results.length - 1], confidence: 0.4 };
        }
    }
    ...
    analyzeDiceVolatility(history) {
        ...
        if (faceSequences.length === 0) return { prediction: null, confidence: 0 };
        ...
        if (predictions.length > 0) {
            ...
            if (topFaces.length >= 2) {
                ...
                if (predictedScores.length > 0) {
                    ...
                    return { prediction: avgPredicted >= 11 ? 'Tài' : 'Xỉu', confidence: 0.65 };
                }
            } else {
                const face = topFaces[0].face;
                return { prediction: (face + 7) >= 11 ? 'Tài' : 'Xỉu', confidence: 0.55 };
            }
        }
        
        return { prediction: history[history.length - 1].Ket_qua || (history[history.length - 1].score >= 11 ? 'Tài' : 'Xỉu'), confidence: 0.4 };
    }
    ...
    ensembleModels(history) {
        ...
        if (totalWeight > 0) {
            ...
            if (taiRatio > 0.55) { finalPrediction = 'Tài'; finalConfidence = taiRatio; }
            else if (xiuRatio > 0.55) { finalPrediction = 'Xỉu'; finalConfidence = xiuRatio; }
            else {
                const bestModel = details[0];
                if (bestModel) { finalPrediction = bestModel.prediction; finalConfidence = 0.5 + bestModel.confidence * 0.2; }
                else { finalPrediction = history.length > 0 ? (history[history.length - 1].Ket_qua || (history[history.length - 1].score >= 11 ? 'Tài' : 'Xỉu')) : 'Tài'; finalConfidence = 0.5; }
            }
        } else {
...
// ==================== HTTP POLLING CHO API MỚI ====================
const DATA_API_URL = "https://sunlaymaynkx.hacksieucap.pro/sunlon123";
...
async function fetchDataAndAnalyze() {
    try {
        ...
        // 1. Trích xuất mảng lịch sử linh hoạt từ API
        let historyArray = [];
        if (Array.isArray(rawData)) {
            historyArray = rawData;
        } else if (rawData.data && Array.isArray(rawData.data)) {
            historyArray = rawData.data;
        } else {
            for (let key in rawData) {
                if (Array.isArray(rawData[key])) {
                    historyArray = rawData[key];
                    break;
                }
            }
        }
        ...
        // 2. Chuẩn hóa Data (Ánh xạ các trường từ API mới về định dạng quen thuộc của thuật toán)
        let standardizedHistory = historyArray.map(item => {
            let phien = item.phien || item.id || item.session || item.Phien;
            let d1 = item.d1 || item.dice1 || item.xx1 || item.Xuc_xac_1 || item.xucxac1 || 1;
            let d2 = item.d2 || item.dice2 || item.xx2 || item.Xuc_xac_2 || item.xucxac2 || 1;
            let d3 = item.d3 || item.dice3 || item.xx3 || item.Xuc_xac_3 || item.xucxac3 || 1;
            let tong = item.tong || item.total || item.Tong || (Number(d1) + Number(d2) + Number(d3));
            let kq = item.kq || item.ketqua || item.Ket_qua || item.result || (tong > 10 ? 'Tài' : 'Xỉu');

            return { phien, Xuc_xac_1: Number(d1), Xuc_xac_2: Number(d2), Xuc_xac_3: Number(d3), Tong: Number(tong), Ket_qua: kq };
        });

        // Đảm bảo mảng sắp xếp từ cũ đến mới (nếu API trả mới nhất ở đầu thì phải đảo ngược)
        if (standardizedHistory[0].phien > standardizedHistory[standardizedHistory.length - 1].phien) {
            standardizedHistory.reverse();
        }

        // Lấy phiên mới nhất
        const latestItem = standardizedHistory[standardizedHistory.length - 1];
        if (!latestItem || !latestItem.phien) return;

        // 3. Nếu là phiên mới hoặc vừa khởi động hệ thống
        if (latestItem.phien !== lastProcessedPhien) {
            
            // Ép thẳng toàn bộ lịch sử API vào ResultHistory để ko cần chờ 5 phiên
            resultHistory = standardizedHistory.slice(-100); 
            saveHistory(latestItem); // Lưu vào cache
            
            lastProcessedPhien = latestItem.phien;

            // Chuyển đổi data để bơm vào Machine Learning Engine
            ...
            // 5. RENDER KẾT QUẢ THEO YÊU CẦU
            apiResponseData = {
                "Phien_truoc": latestItem.phien,
                "Ket_qua_phien_truoc": latestItem.Ket_qua,
                "Xuc_sac_phien_truoc": `${latestItem.Xuc_xac_1} - ${latestItem.Xuc_xac_2} - ${latestItem.Xuc_xac_3}`,
                "Phien_nay": Number(latestItem.phien) + 1,
            ...
            console.log('\n' + '🟦'.repeat(20));
            console.log(`✅ [DATA NHẬN MỚI] Đã phân tích thành công!`);
            console.log(`🎲 Phiên trước: ${apiResponseData.Phien_truoc} | Kết quả: ${apiResponseData.Ket_qua_phien_truoc}`);
            console.log(`🎲 Xúc xắc: ${apiResponseData.Xuc_sac_phien_truoc}`);
            console.log(`🤖 Phiên nay (${apiResponseData.Phien_nay}) | Dự đoán: ${apiResponseData.Du_doan}`);
            console.log(`🔍 Cầu: ${apiResponseData.Loai_cau} | Tỉ lệ ăn: ${apiResponseData.Do_tin_cay}`);
            console.log('🟦'.repeat(20) + '\n');
        }
    } catch (error) {
        // Lỗi kết nối mạng hoặc API chết
        console.error('[❌] Lỗi khi lấy hoặc xử lý dữ liệu từ API:', error.message);
    }
}

// Chạy lần đầu ngay lập tức
fetchDataAndAnalyze();
// Quét dữ liệu liên tục mỗi 3 giây
...
app.get('/api/models', (req, res) => {
    res.json({
        ...
        total: 21 + 42 + 21,
        status: "Đang chạy 100% tài nguyên phân tích"
    });
});

app.listen(PORT, () => {
    console.log(`[🌐] Server is running at http://localhost:${PORT}`);
    console.log(`[🔗] Đã kết nối HTTP Polling với API Data Mới`);
    console.log(`[🤖] Total models: 21 main + 42 sub + 21 mini = 84 models (Đang hoạt động)`);
    console.log(`[🚀] Tính năng ép Lịch Sử 0s đã KÍCH HOẠT. Phân tích ngay không cần chờ 5 phiên.`);
});